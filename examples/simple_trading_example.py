#!/usr/bin/env python3
"""
Simple Trading Example - Interactive Brokers API
...
class SimpleTradingApp(EWrapper, EClient):
    ...
    def connect_and_run(self, host: str = "127.0.0.1", port: int = 4001, client_id: int = 1):
        ...
        try:
            self.logger.info(f"🔌 Connecting to {host}:{port} (Client ID: {client_id})")
            ...
            if self.isConnected():
                ...
                if self.next_order_id is None:
                    self.logger.error("❌ Failed to receive next valid order ID")
                    return False
    ...
    def get_positions(self):
        ...
        if self.position_event.wait(timeout=10):
            self.logger.info("✅ Position data received")
        else:
            self.logger.warning("⏰ Timeout waiting for position data")

def main():
    """Main trading demonstration"""
    print("🚀 Simple Trading Example - Interactive Brokers API")
    print("📝 This example demonstrates placing trades and viewing positions")
    print("⚠️  IMPORTANT: Make sure IB Gateway is running on port 4001 (paper trading)")
    print("\n" + "="*80)
    ...
    try:
        ...
        print("\n1️⃣ Placing market buy order for 1 share of AAPL...")
        ...
        print("\n2️⃣ Placing limit buy order for 1 share of MSFT at $250...")
        ...
        print("\n✅ TRADING DEMONSTRATION COMPLETE")
