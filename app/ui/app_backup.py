...
def get_standalone_account_info():
    ...
    if st.session_state.ibkr_connected and st.session_state.standalone_api:
        try:
            ...
            # Calculate real P&L from positions and account data
            # Get total unrealized P&L from all positions
            total_unrealized_pnl = 0.0
            total_realized_pnl = 0.0
            
            # Request positions data to get real P&L
            ...
            for symbol, ib_position in api.positions.items():
                if ib_position.position != 0:
                    total_unrealized_pnl += ib_position.unrealized_pnl
                    # Note: IBPosition object only has unrealized_pnl, not realized_pnl
                    # Realized P&L will be obtained from account summary data
            
            # Get daily P&L from account summary if available
            daily_pnl = account_info.get('DailyPnL', total_unrealized_pnl)
            unrealized_pnl = account_info.get('UnrealizedPnL', total_unrealized_pnl)
            realized_pnl = account_info.get('RealizedPnL', total_realized_pnl)
            
            # Use the most accurate P&L data available
            if abs(daily_pnl) > 0:
                pnl = daily_pnl
            elif abs(unrealized_pnl) > 0:
                pnl = unrealized_pnl + realized_pnl
            else:
                pnl = total_unrealized_pnl + total_realized_pnl
            
            # Calculate P&L percentage based on current equity minus P&L
            base_equity = net_liquidation - pnl
            pnl_percent = (pnl / base_equity * 100) if base_equity > 0 else 0.0
        ...
        try:
            ...
            for symbol, ib_position in api.positions.items():
                if ib_position.position != 0:  # Only show non-zero positions
                    ...
                    unrealized_pnl = ib_position.unrealized_pnl
                    unrealized_pnl_percent = (unrealized_pnl / (abs(ib_position.position) * ib_position.avg_cost) * 100) if ib_position.avg_cost > 0 else 0.0
...
st.markdown("""
<div class="footer">
    <p>🤖 MartinGales Trading Bot v1.0 | Built with Streamlit | 
    Last updated: {}</p>
